...
require Exporter;
package Math::VecStat::Fixed;
@Math::VecStat::Fixed::ISA=qw(Exporter);
...
{
	my $a = $Math::VecStat::Fixed::VERSION;
}
...
=head1 NAME
    ...
    Math::VecStat::Fixed - Some basic numeric stats on vectors

=head1 SYNOPSIS

    use Math::VecStat::Fixed qw(max min maxabs minabs sum average);
    $max=max(@vector);
    $max=max(\@vector);
    ($max,$imax)=max(@vector);
    ($max,$imax)=max(\@vector);
...
median element in the original vector.
  ...
  $a = Math::VecStat::Fixed::median( [9,8,7,6,5,4,3,2,1] );
